    ...
    const tools = {
        perform_login: {
            ...
            execute: async ({ site }: { site: string }) => {
                ...
                const { username, password, totpSecret, carrier } = credentials;
                ...
                try {
                    ...
                    if (totpSecret || carrier) {
                        ...
                        if (tfaCheck.requires2FA) {
                            ...
                            // Handle email-based 2FA (when carrier is provided and no totpSecret)
                            else if (carrier) {
                                ...
                                for (let codeAttempt = 1; codeAttempt <= maxCodeAttempts; codeAttempt++) {
                                    ...
                                    console.log(`[perform_login] Fetching email 2FA code for provider: ${carrier}...`);
                                    ...
                                    for (let fetchAttempt = 1; fetchAttempt <= 10; fetchAttempt++) {
                                        ...
                                        emailCode = await fetchEmail2FACode(carrier);
                                        if (emailCode) {
